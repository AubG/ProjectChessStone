...
public class InvDatabaseInspector : Editor
{
	...
	InvBaseItem.InteractType mSetType = InvBaseItem.InteractType.Passive;
		...
		else
		{
			...
			if (GUILayout.Button("New Item"))
			{
				...
				if (item != null)
				{
					...
					foreach (InvComponent component in item.components)
					{
						//bi.components.Add(
					}
				}
			...
			if (item != null)
			{
				...
				GUILayout.BeginHorizontal();
				{
					...
					EditorGUILayout.LabelField("(ID: " + item.id16 + ")");
				...
				{
					// Show the confirmation dialog
					GUILayout.Label("Change interaction type?");
					...
					GUILayout.BeginHorizontal();
					{
						...
						{
							NGUIEditorTools.RegisterUndo("Change Item Type", db);
							item.interactType = mSetType;
				...
				{
					InvBaseItem.InteractType type = item.interactType;
					
					GUILayout.BeginHorizontal();
					{
						EditorGUILayout.LabelField("Interaction Component", EditorStyles.boldLabel);
						
						bool alreadySet = false;
						foreach(InvBaseItem.InteractType typeVal in Enum.GetValues(typeof(InvBaseItem.InteractType))) {
							alreadySet = item.interactType == typeVal;
					...
					switch(type) {
						case InvBaseItem.InteractType.Active:
							break;
						case InvBaseItem.InteractType.Equipment:
							EquipComponent equip = item.equip;
							
							EquipSlot slot = (EquipSlot)EditorGUILayout.EnumPopup("Slot", equip.slot);
							
							// Game Object attachment field, left of the icon
							GameObject go = (GameObject)EditorGUILayout.ObjectField("Attachment", equip.attachment, typeof(GameObject), false);
							
							if (equip.stats != null)
							{
								for (int i = 0; i < equip.stats.Count; ++i)
								{
									InvStat stat = equip.stats[i];
							...
							if (slot	!= equip.slot ||
								go		!= equip.attachment)
							{
								NGUIEditorTools.RegisterUndo("Equip Properties", db);
								equip.slot = slot;
								equip.attachment = go;

using UnityEngine;
using System.Collections.Generic;

[ExecuteInEditMode]
[AddComponentMenu("Spells/Spell Database")]
public class SpellDatabase : MonoBehaviour
{
	// Cached list of all available item databases
	static SpellDatabase[] mList;
	static bool mIsDirty = true;

	/// <summary>
	/// Retrieves the list of item databases, finding all instances if necessary.
	/// </summary>

	static public SpellDatabase[] list
	{
		get
		{
			if (mIsDirty)
			{
				mIsDirty = false;
				mList = NGUITools.FindActive<SpellDatabase>();
			}
			return mList;
		}
	}

	/// <summary>
	/// Each database should have a unique 16-bit ID. When the items are saved, database IDs
	/// get combined with item IDs to create 32-bit IDs containing both values.
	/// </summary>

	public int databaseID = 0;

	/// <summary>
	/// List of items in this database.
	/// </summary>

	public List<BaseSpell> spells = new List<BaseSpell>();

	/// <summary>
	/// UI atlas used for icons.
	/// </summary>

	public UIAtlas iconAtlas;

	/// <summary>
	/// Add this database to the list.
	/// </summary>

	void OnEnable () { mIsDirty = true; }

	/// <summary>
	/// Remove this database from the list.
	/// </summary>

	void OnDisable () { mIsDirty = true; }

	/// <summary>
	/// Find an item by its 16-bit ID.
	/// </summary>

	BaseSpell GetSpell (int id16)
	{
		for (int i = 0, imax = spells.Count; i < imax; ++i)
		{
			BaseSpell spell = spells[i];
			if (spell.id16 == id16) return spell;
		}
		return null;
	}

	/// <summary>
	/// Find a database given its ID.
	/// </summary>

	static SpellDatabase GetDatabase (int dbID)
	{
		for (int i = 0, imax = list.Length; i < imax; ++i)
		{
			SpellDatabase db = list[i];
			if (db.databaseID == dbID) return db;
		}
		return null;
	}

	/// <summary>
	/// Find the specified item given its full 32-bit ID (not to be confused with individual 16-bit item IDs).
	/// </summary>

	static public BaseSpell FindByID (int id32)
	{
		SpellDatabase db = GetDatabase(id32 >> 16);
		return (db != null) ? db.GetSpell(id32 & 0xFFFF) : null;
	}

	/// <summary>
	/// Find the item with the specified name.
	/// </summary>

	static public BaseSpell FindByName (string exact)
	{
		for (int i = 0, imax = list.Length; i < imax; ++i)
		{
			SpellDatabase db = list[i];

			for (int b = 0, bmax = db.spells.Count; b < bmax; ++b)
			{
				BaseSpell spell = db.spells[b];

				if (spell.name == exact)
				{
					return spell;
				}
			}
		}
		return null;
	}

	/// <summary>
	/// Get the full 32-bit ID of the specified item.
	/// Use this to get a list of items on the character that can get saved out to an external database or file.
	/// </summary>

	static public int FindSpellID (BaseSpell spell)
	{
		for (int i = 0, imax = list.Length; i < imax; ++i)
		{
			SpellDatabase db = list[i];

			if (db.spells.Contains(spell))
			{
				return (db.databaseID << 16) | spell.id16;
			}
		}
		return -1;
	}
}
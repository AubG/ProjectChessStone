...
public class InvCardDatabase : MonoBehaviour
{
	// Cached list of all available item databases
	static InvCardDatabase[] mList;
	...
	static public InvCardDatabase[] list
	{
		get
		{
			if (mIsDirty)
			{
				mIsDirty = false;
				mList = NGUITools.FindActive<InvCardDatabase>();
			}
			return mList;
		}
	}
	...
	public List<InvBaseCard> items = new List<InvBaseCard>();
	...
	InvBaseCard GetItem (int id16)
	{
		for (int i = 0, imax = items.Count; i < imax; ++i)
		{
			InvBaseCard item = items[i];
	...
	static InvCardDatabase GetDatabase (int dbID)
	{
		for (int i = 0, imax = list.Length; i < imax; ++i)
		{
			InvCardDatabase db = list[i];
	...
	static public InvBaseCard FindByID (int id32)
	{
		InvCardDatabase db = GetDatabase(id32 >> 16);
	...
	static public InvBaseCard FindByName (string exact)
	{
		for (int i = 0, imax = list.Length; i < imax; ++i)
		{
			InvCardDatabase db = list[i];
	...
	static public int FindItemID (InvBaseCard item)
	{
		for (int i = 0, imax = list.Length; i < imax; ++i)
		{
			InvCardDatabase db = list[i];

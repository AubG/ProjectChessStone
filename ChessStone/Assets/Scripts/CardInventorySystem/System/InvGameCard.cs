...
public class InvGameCard
{
	...
	public InvBaseCard baseItem
	{
		get
		{
			if (mBaseItem == null)
			{
				mBaseItem = InvCardDatabase.FindByID(baseItemID);
			}
			return mBaseItem;
		}
	}
	...
	public InvGameCard (int id) { mBaseItemID = id; }

	/// <summary>
	/// Create a game item with the specified ID and base item.
	/// </summary>

	public InvGameCard (int id, InvBaseCard bi) { mBaseItemID = id; mBaseItem = bi; }

	/// <summary>
	/// Calculate and return the list of effective stats based on item level and quality.
	/// </summary>

	public List<InvCard> CalculateStats ()
	{
		List<InvCard> stats = new List<InvCard>();

		if (baseItem != null)
		{
			float mult = statMultiplier;
			List<InvCard> baseStats = baseItem.stats;

			for (int i = 0, imax = baseStats.Count; i < imax; ++i)
			{
				InvCard bs = baseStats[i];
			...
			//stats.Sort(InvCard.CompareArmor);
		}
		return stats;
	}
}
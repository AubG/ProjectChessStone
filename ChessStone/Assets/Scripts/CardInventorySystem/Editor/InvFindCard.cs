...
public class InvFindCard : ScriptableWizard
{
	/// <summary>
	...
	struct FindResult
	{
		public InvCardDatabase db;
		public InvBaseCard item;
	}
	...
	{
		ScriptableWizard.DisplayWizard<InvFindCard>("Find Item");
	}
	...
	void OnGUI ()
	{
		...
		else
		{
			...
			{
				if (Print3(InvCardDatabase.FindItemID(fr.item).ToString(),
					fr.item.name, NGUITools.GetHierarchy(fr.db.gameObject), true))
				{
					CardInvDatabaseInspector.SelectIndex(fr.db, fr.item);
	...
	void FindAllByName (string partial)
	{
		...
		foreach (InvCardDatabase db in InvCardDatabase.list)
		{
			foreach (InvBaseCard item in db.items)
			{

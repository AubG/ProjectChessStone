...
//Mem - 4+1+4+1+[4]+[4]+1+1+4+4+4+4+4+  12+12+12+12+12+12+4+4+4+4+4+1+1+(4)+4+4+4+4+4+4+4 ? 166 bytes
	...
	public class ABTilePath : Path {
		...
		public Tile originalStartTile;
		
		/** End Point exactly as in the path request */
		public Tile originalEndTile;
		...
		public ABTilePath (Tile start, Tile end, OnPathDelegate callbackDelegate) {
			Reset ();
		...
		protected void Setup (Tile start, Tile end, OnPathDelegate callbackDelegate) {
		...
		protected void UpdateStartEnd (Tile start, Tile end) {
			originalStartTile = start;
			originalEndTile = end;
			
			startTile = start;
			endTile = end;

			hTarget = (Int3)end.centerPos;
		}
		...
		public override void Reset () {
			...
			startHint = null;
			endHint = null;
			originalStartTile = null;
			originalEndTile = null;
			startTile = null;
			endTile = null;
		...
		public override void Prepare () {
			...
			startNode = graph.nodes[startTile.y * graph.Width + startTile.x];
			endNode = graph.nodes[endTile.y * graph.Width + endTile.x];
		 ...
		 * 
		 * Basic outline of what the function does for the standard path (Pathfinding.ABTilePath).
\code
...
\endcode
		...
		protected override void Recycle () {
			PathPool<ABTilePath>.Recycle (this);
		}
